...
//8.51 mul 2.5 mux
	...
	if(load_word(par_start_address) == 1 && loopflag == 1 && endflag == 0 && core.memoryreg.pc>=(par_start_change_pc+0x20)){ //note changed values
	...
	if (memoryreg.isopcodeBranchorJump) { // bne is must be guaranteed
		...
		if(changed_register_Arr[memoryreg.rs2]){
			start_reg= memoryreg.rs2 ;
			end_reg= memoryreg.rs1 ;
		}else{
			start_reg= memoryreg.rs1 ;
			end_reg= memoryreg.rs2 ;
		}
